...
if ! $sflag
then
    S3_BUCKET=ibk-ml-$SOURCE_REGION-$SOURCE_ACCOUNT-glue-data-catalog-replication
fi
if ! $nflag
then
    STACK_NAME="ibk-ml-glue-data-catalog-replication-source"
fi
...
--action-name Subscribe ListSubscriptionsByTopic Receive --profile ${PROFILE}
